...
void PlateDetector::postProcess(cv::Mat &bboxes_raw, cv::Mat &scores_raw, vector<cv::RotatedRect> &bboxes, vector<float> &scores)
{
    double r_min, r_max, c_min, c_max, w, h, angle;
    ...
    for (int i = 0; i < bboxes_raw.size[1]; i++)
    {
        ...
        {              
            if(scores_raw.at<float>(0, i, j) > score_threshold)
            {
                ...
                bboxes.push_back(cv::RotatedRect(cv::Point2f(c_min + w/2, r_min + h/2), cv::Size2f(w, h), angle + 1.0));
